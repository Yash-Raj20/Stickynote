...
export default function TemplatesPage() {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-background">
      {/* Header */}
      <div className="px-4 md:px-8 pt-7 pb-3 md:pb-5 border-b border-border shrink-0">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2.5">
            <LayoutTemplate size={20} className="text-theme-primary shrink-0" />
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto custom-scrollbar pb-2 sm:pb-0 sm:flex-wrap">
          {categories.map(cat => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
              className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${activeCategory === cat.id ? 'bg-theme-primary text-white' : 'bg-input-bg text-foreground/60 hover:text-foreground hover:bg-border'
                }`}
            >
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-5 flex flex-col gap-6">
        {/* Recently Used */}
    ...
    <div className="group relative bg-surface border border-border rounded-xl overflow-hidden hover:border-theme-primary/40 hover:shadow-md transition-all duration-200 flex flex-col cursor-pointer" onClick={onPreview}>
      ...
      {isCustom && onDelete && (
        <button onClick={e => { e.stopPropagation(); onDelete(); }}
          ...
          className="absolute top-2 right-2 z-10 p-1.5 rounded-lg text-white/90 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 bg-red-500/80 hover:bg-red-500 backdrop-blur-sm transition-all shadow-sm">
          <Trash2 size={16} />
        </button>
      )}
      <div className="relative h-48 bg-input-bg overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] hidden lg:flex flex-col gap-3 items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200">
      ...
      <div className="px-4 py-3 flex flex-col gap-1">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <button onClick={e => { e.stopPropagation(); onUse(); }} disabled={loading}
            className="lg:hidden flex items-center justify-center w-7 h-7 bg-theme-primary text-white rounded-lg shadow-sm active:scale-95 transition-all">
...
function PreviewModal({ template, loading, isFavorite, onClose, onUse, onToggleFav }: {
  ...
  useEffect(() => {
    const updateDim = () => {
      ...
      // padding: p-4 on mobile (32px total), p-8 on desktop (64px total)
      const W = isMobile ? window.innerWidth - 32 : Math.min(760, window.innerWidth - 64);
      // Header ~ 70px, Footer ~ 70px, Paddings ~ 60px = 200px
      const H = isMobile ? window.innerHeight - 200 : Math.min(440, window.innerHeight - 200);
      setDim({ W, H: Math.max(200, H) });
    };
    updateDim();
  ...
  const scale = computeScale(template.notes, dim.W, dim.H);
  return (
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-8" onClick={onClose}>
      <div className="bg-surface rounded-2xl w-full max-w-4xl max-h-[95vh] shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-start sm:items-center justify-between px-4 sm:px-6 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            <span className="text-xl sm:text-2xl">{template.emoji}</span>
            <div>
              <h2 className="font-bold text-base sm:text-lg text-foreground leading-none">{template.name}</h2>
              <p className="text-xs sm:text-sm text-foreground/50 mt-0.5 line-clamp-1">{template.description}</p>
            </div>
          </div>
          <div className="flex items-center gap-1 sm:gap-2 shrink-0 ml-2">
            <button onClick={onToggleFav}
              className={`p-1.5 sm:p-2 rounded-lg transition-all ${isFavorite ? 'text-theme-primary bg-theme-secondary' : 'text-foreground/40 hover:text-foreground hover:bg-input-bg'}`}>
              <Star size={16} fill={isFavorite ? 'currentColor' : 'none'} />
            </button>
            <button onClick={onClose} className="p-1.5 sm:p-2 rounded-lg text-foreground/40 hover:text-foreground hover:bg-input-bg transition-colors"><X size={16} /></button>
          </div>
        </div>
        <div className="bg-input-bg relative overflow-hidden" style={{ height: dim.H }}>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-border flex items-center justify-between">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1 sm:gap-3">
            <span className={`inline-flex items-center gap-1 text-[10px] sm:text-xs font-medium px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg ${CAT_BADGE[template.category] || 'bg-input-bg text-foreground/50'}`}>
          ...
          <button onClick={onUse} disabled={loading}
            className="flex items-center gap-2 bg-theme-primary text-white font-semibold px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl hover:opacity-90 transition-opacity disabled:opacity-60 text-xs sm:text-sm shrink-0">
            {loading ? <><Loader2 size={15} className="animate-spin" />Loading...</> : <>Use Template<ArrowRight size={15} /></>}
          </button>
        </div>
      </div>
